...
obstime=10000000.0      #length of observation (in seconds)
back=0.00    #constant background (same in all energy bands)
...
rho=3.14159  # in radians
phaseshift=0.0 # this is used when comparing with data
temp=0.05 # in keV (Surface temperature)
...
# TEST 1: Cole's McPHAC, theory
out_file="$out_dir/mcphacc_may12_hbg.txt"
## RUNNING THE CODE
./spot -m "$mass" -r "$radius" -f "$spin" -i "$inclination" -e "$emission" -l "$phaseshift" -n "$numbins" -q "$NS_model" -o "$out_file" -p "$rho" -T "$temp" -D "$distance" -t "$numtheta" -g "$beaming" -s "$spectraltype" -S "$numbands" -u "$elo" -U "$ehi" -P "$spotmodel" -b "angles1000.txt" -Z "$obstime" -R "$inst_res" -k "$back" 
times