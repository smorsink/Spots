	...
	*/
	...
	//		for (unsigned int k(16); k < 18; k++) { // Loop through the circles
	  ...
	  for ( unsigned int j(0); j < numphi; j++ ) {// looping through the phi divisions
	    ...
	    //curve.para.phi_0 =  -phi_edge + (j+0.5)*dphi; This is Version 1

	    curve.para.phi_0 =  -phi_edge + (j+0.5)*dphi + 0.5*phishift;	// This version 2		
	  ...
	  if (phishift != 0.0){ 
	    ...
	    // Add light from first bin, which requires shifting
	    for ( unsigned int i(0); i < numbins; i++ ) {
	      int q(i-1);
	      if (q<0) q+=numbins;
	    ...
	    curve = ShiftCurve(&curve,0.5*phishift);
       
	    for( unsigned int pp(0); pp < numbands; pp++ )
	      for ( unsigned int i(0); i < numbins; i++ ) Flux[pp][i] += curve.f[pp][i]*0.5*phishift/dphi      ;
	    //end of last bin


	    // Add light from last bin, which requires shifting
	    for ( unsigned int i(0); i < numbins; i++ ) {
	      int q(i+numphi-1);
	      if (q>=numbins) q+=-numbins;
	      for ( unsigned int pp(0); pp < numbands; pp++ ) {
