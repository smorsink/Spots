...
double McPHACC3new(double E, double cos_theta, int theta_index, double T, double lgrav, int i_lgrav, double gvec[4], class LightCurve* curve){
  ...
  mexmcc = (*curve);
	...
	//if (cos_theta < 0.00015629) L = 0;
	//if (cos_theta < 0.00000001) L = 0;
	//else{
    ...
    L = polint(tvec,K,tpt,lt,&err)*pow(10.0,lt*3.0);
...
double AtmosEBandFlux3new( unsigned int model, double cos_theta, int theta_index, double T, double lgrav,  int i_lgrav, double gvec[4], double E1, double E2, class LightCurve mexmcc){
    ...
    if (n_steps == 0){ // zero energy points within bandwidth: (4.1.3) one trapzoid
      //cout << "0 steps" << endl;
      flux = (E2 - E1) / 2.0 * (McPHACC3new(E1,cos_theta,theta_index, T, lgrav, i_lgrav, gvec, &mexmcc) / E1 + McPHACC3new(E2,cos_theta,theta_index, T, lgrav, i_lgrav, gvec, &mexmcc) / E2);
    }
	...
	double counts_m = McPHACC3new(e_m,cos_theta,theta_index, T,lgrav, i_lgrav, gvec, &mexmcc) / e_m;

        flux = (e_m - E1) / 2 * (McPHACC3new(E1,cos_theta,theta_index, T, lgrav, i_lgrav, gvec, &mexmcc) / E1 
				 + counts_m);  // first trapezoid
        flux += (E2 - e_m) / 2 * (counts_m + McPHACC3new(E2,cos_theta, theta_index, T,lgrav,i_lgrav,gvec, &mexmcc) / E2); // second trapezoid
    }
    if (n_steps >= 2){ // two energy points within bandwidth: (4.1.3) three trapzoids
      //cout << "n steps= " << n_steps << endl;
       ...
       flux = e_step * 0.5 * McPHACC3new(E1,cos_theta, theta_index, T, lgrav,i_lgrav, gvec, &mexmcc) / E1;
       //cout << "flux = " << flux << endl;

       for (int i(1); i<=n_steps; i++){

	 e_l = E1 + i*e_step;
	 counts_l = McPHACC3new(e_l,cos_theta,theta_index, T, lgrav,i_lgrav, gvec, &mexmcc) / e_l;
	 flux += e_step * (counts_l);
	 //cout << "flux = " << flux << endl;

       }

       flux += e_step * 0.5 *  McPHACC3new(E2,cos_theta, theta_index, T,lgrav,i_lgrav, gvec, &mexmcc) / E2;
       // cout << "flux = " << flux << endl;
